...
void MeshRendererComponent::SetMesh(const std::string &meshPath) {
  ...
  auto model = LoadModel(meshPath);
  if (model) {
    loadedModel = std::make_shared<Model>(std::move(*model));
  } else {
    std::cerr << "Failed to load model: " << meshPath << std::endl;
  }
}
...
glm::mat4 CameraComponent::GetProjectionMatrix() const {
  if (projectionType == ProjectionType::Perspective) {
    return glm::perspective(glm::radians(fieldOfView), aspectRatio,
...
glm::mat4 CameraComponent::GetViewMatrix() const {
  ...
  glm::vec3 forward =
      rotation * glm::vec3(0, 0, -1); // Forward is -Z in camera space
...
AudioComponent::AudioComponent(Actor *owner) : SceneComponent(owner) {
  bCanTick = false;
}
